...
import static android.provider.Settings.Secure.SCANDIUM_VOLUME_HUSH_MUTE;
import static android.provider.Settings.Secure.SCANDIUM_VOLUME_HUSH_NORMAL;
import static android.provider.Settings.Secure.SCANDIUM_VOLUME_HUSH_OFF;
import static android.provider.Settings.Secure.SCANDIUM_VOLUME_HUSH_VIBRATE;
...
public class PreventRingingGesturePreferenceController extends AbstractPreferenceController
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (settingsValue == null || settingsValue.isEmpty()) settingsValue = SCANDIUM_VOLUME_HUSH_OFF;
        final ArrayList<String> currentValue = new ArrayList<>(Arrays.asList(settingsValue.split(",", 3)));

        if (isAdd) {
            if (currentValue.get(0).equals(SCANDIUM_VOLUME_HUSH_OFF))
                currentValue.clear();
            if (!currentValue.contains(preventRingingSetting))
                currentValue.add(preventRingingSetting);
        } else {
            if (currentValue.size() == 1 ||
                    preventRingingSetting.equals(SCANDIUM_VOLUME_HUSH_OFF)) {
                currentValue.clear();
                currentValue.add(SCANDIUM_VOLUME_HUSH_OFF);
    ...
    public void updateState(Preference preference) {
        ...
        final boolean isVibrate = enabled && preventRingingSetting.contains(SCANDIUM_VOLUME_HUSH_VIBRATE);
        final boolean isMute = enabled && preventRingingSetting.contains(SCANDIUM_VOLUME_HUSH_MUTE);
        final boolean isNormal = enabled && preventRingingSetting.contains(SCANDIUM_VOLUME_HUSH_NORMAL);
    ...
    private String keyToSetting(String key) {
        switch (key) {
            case KEY_MUTE:
                return SCANDIUM_VOLUME_HUSH_MUTE;
            case KEY_VIBRATE:
                return SCANDIUM_VOLUME_HUSH_VIBRATE;
            case KEY_NORMAL:
                return SCANDIUM_VOLUME_HUSH_NORMAL;
            default:
                return SCANDIUM_VOLUME_HUSH_OFF;
        }
    }
