...
package com.android.settings.scandium;
...
public class TopMenuBattery extends Preference implements
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        PowerUsageFeatureProvider mPowerFeatureProvider = FeatureFactory.getFactory(context)
                .getPowerUsageFeatureProvider(context);
        mBatteryUsageSummary.setText(
                mPowerFeatureProvider.isChartGraphEnabled(context) ?
                        context.getString(R.string.advanced_battery_preference_summary_with_hours) :
                        context.getString(R.string.advanced_battery_preference_summary));
        ...
        boolean blurStyleEnable = mBlurStyleEnable = Settings.System.getIntForUser(context.getContentResolver(),
                Settings.System.BLUR_STYLE_PREFERENCE_KEY, 0, UserHandle.USER_CURRENT) == 1;
        if (blurStyleEnable) {
            Drawable notifBg = mTopBattery.getBackground();
            if (notifBg != null) notifBg.setAlpha(100);
            Drawable conBg = mBatteryProgressBar.getBackground();
            if (conBg != null) conBg.setAlpha(100);
            Drawable bubbleBg = mBatteryUsage.getBackground();
            if (bubbleBg != null) bubbleBg.setAlpha(100);
        }
    }
    ...
    public int colorTint() {
    	int colorAccent = Utils.getColorAttrDefaultColor(getContext(), android.R.attr.colorAccent);
    	return mBlurStyleEnable ? Utils.applyAlpha(0.4f, colorAccent) : colorAccent;
    }
